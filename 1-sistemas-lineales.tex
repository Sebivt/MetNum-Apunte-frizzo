...
le suma $\lambda$ veces la fila $j$-ésima (la columna $i$-ésima) a la
...
1$ tienen ceros debajo de la diagonal, asegurando que, tras $n-1$ iteraciones,
...
\[ a^{(k)}_{i,k} = a^{(k-1)}_{i,k} - m^{(k)}_i \cdot a^{(k-1)}_{k,k}, \]
